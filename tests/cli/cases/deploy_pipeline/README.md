# How this repo works
1. `data_load_tool init <source> <destination>` clones this repo, it uses the version of the data_load_tool as a git tag to clone
...
4. it copies the .gitignore, the pipeline script created above, the README and requirements to the folder in which data_load_tool was called
5. it will create the `secrets.toml` and `config.toml` for the source and destination in the script
6. it will add the right data_load_tool extra to `requirements.txt`
