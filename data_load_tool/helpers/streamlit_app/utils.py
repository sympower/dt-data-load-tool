...
def render_with_pipeline(render_func: Callable[..., None]) -> None:
    ...
    pipeline = data_load_tool.attach(pipeline_name, pipelines_dir=pipelines_dir)
    render_func(pipeline)


def query_using_cache(pipeline: data_load_tool.Pipeline, ttl: int) -> Callable[..., Optional[pd.DataFrame]]:
