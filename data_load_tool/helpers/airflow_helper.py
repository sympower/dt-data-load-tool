...
from data_load_tool.common.known_env import DLT_DATA_DIR, DLT_PROJECT_DIR, DLT_LOCAL_DIR
...
from data_load_tool.common.runtime.telemetry import with_telemetry

from data_load_tool.common.destination import TLoaderFileFormat
from data_load_tool.common.schema.typing import TWriteDispositionConfig, TSchemaContract
...
from data_load_tool.common.runtime.collector import NULL_COLLECTOR
...
from data_load_tool.pipeline.progress import log
...
class PipelineTasksGroup(TaskGroup):
    """
    Represents a data_load_tool Airflow pipeline task group.
    """
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        - the .data_load_tool folder (the project folder) is searched under `dags` as configured by Airflow
        ...
        `abort_task_if_any_job_failed` will abort the other data_load_tool loading jobs and fail the Airflow task in any of the jobs failed. See https://dlthub.com/docs/running-in-production/running#handle-exceptions-failed-jobs-and-retry-the-pipeline.
        ...
        Args:
            ...
            use_task_logger (bool, optional): Will redirect data_load_tool logger into task logger. Defaults to True.
            ...
            buffer_max_items (int, optional): Maximum number of buffered items. Use 0 to keep data_load_tool built-in limit. Defaults to 1000.
        ...
        # set the data_load_tool project folder to dags
    ...
    def _run(
        ...
        if data_load_tool.config.get("data_writer.buffer_max_items") is None and self.buffer_max_items > 0:
            data_load_tool.config["data_writer.buffer_max_items"] = self.buffer_max_items
            logger.info(f"Set data_writer.buffer_max_items to {self.buffer_max_items}")

        if self.abort_task_if_any_job_failed is not None:
            data_load_tool.config["load.raise_on_failed_jobs"] = self.abort_task_if_any_job_failed
            logger.info(
    ...
    def add_run(
        ...
        **kwargs: Any,
        ...
        if not pipeline.pipelines_dir.startswith(os.environ[DLT_DATA_DIR]):
            raise ValueError(
                "Please create your Pipeline instance after AirflowTasks are created. The data_load_tool"
        ...
        with self:
            ...
            if decompose == "none":
                ...
                if not isinstance(data, DltSource):
                    raise ValueError("Can only decompose data_load_tool sources")
