...
from data_load_tool.common.configuration import with_config, known_sections
...
from data_load_tool.common.runners.stdout import iter_stdout_with_result
from data_load_tool.common.typing import StrAny, TSecretStrValue
from data_load_tool.common.logger import is_json_logging
from data_load_tool.common.storages import FileStorage
from data_load_tool.common.git import git_custom_key_command, ensure_remote_head, force_clone_repo
from data_load_tool.common.utils import with_custom_environ

from data_load_tool.helpers.dbt.configuration import DBTRunnerConfiguration
from data_load_tool.helpers.dbt.exceptions import (
...
class DBTPackageRunner:
    ...
    The created wrapper minimizes the required effort to run `dbt` packages on datasets created with `data_load_tool`. It clones the package repo and keeps it up to data,
    shares the `data_load_tool` destination credentials with `dbt` and allows the isolated execution with `venv` parameter.
    The wrapper creates a `dbt` profile from a passed `data_load_tool` credentials and executes the transformations in `source_dataset_name` schema. Additional configuration is
    ...
    def _run_dbt_command(
        ...
        if self.credentials:
            add_config_to_env(self.credentials, ("data_load_tool",))
        args = [
...
from data_load_tool.helpers.dbt.dbt_utils import init_logging_and_run_dbt_command
...
def create_runner(
    ...
    credentials: DestinationClientDwhConfiguration,
    working_dir: str,
    package_location: str = data_load_tool.config.value,
    ...
    optionally shares the `data_load_tool` destination credentials with `dbt` and allows the isolated execution with `venv` parameter.
