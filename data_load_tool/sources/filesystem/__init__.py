...
from data_load_tool.extract import decorators
...
from data_load_tool.sources.credentials import FileSystemCredentials

from data_load_tool.sources.filesystem.helpers import (
    AbstractFileSystem,
    FilesystemConfigurationResource,
)
from data_load_tool.sources.filesystem.readers import (
    ReadersSource,
    _read_csv,
    _read_csv_duckdb,
    _read_jsonl,
...
from data_load_tool.sources.filesystem.settings import DEFAULT_CHUNK_SIZE


@decorators.source(_impl_cls=ReadersSource, spec=FilesystemConfigurationResource)
def readers(
    bucket_url: str = data_load_tool.secrets.value,
    credentials: Union[FileSystemCredentials, AbstractFileSystem] = data_load_tool.secrets.value,
    ...
    return (
        ...
        | data_load_tool.transformer(name="read_csv")(_read_csv),
        filesystem(bucket_url, credentials, file_glob=file_glob)
        | data_load_tool.transformer(name="read_jsonl")(_read_jsonl),
        filesystem(bucket_url, credentials, file_glob=file_glob)
        | data_load_tool.transformer(name="read_parquet")(_read_parquet),
        filesystem(bucket_url, credentials, file_glob=file_glob)
        | data_load_tool.transformer(name="read_csv_duckdb")(_read_csv_duckdb),
    )
