...
from data_load_tool.common.utils import exclude_keys

from data_load_tool.extract import Incremental, DltResource, DltSource, decorators
...
from data_load_tool.sources.helpers.rest_client.typing import HTTPMethodBasic
from .typing import (
    AuthConfig,
    ClientConfig,
...
@decorators.source
def rest_api(
    client: ClientConfig = data_load_tool.config.value,
    resources: List[Union[str, EndpointResource, DltResource]] = data_load_tool.config.value,
...
def rest_api_source(
    ...
    Args:
        ...
        schema (Schema, optional): An explicit `Schema` instance to be associated
            with the source. If not present, `data_load_tool` creates a new `Schema` object
    ...
    Returns:
        DltSource: A configured data_load_tool source.
...
def rest_api_resources(config: RESTAPIConfig) -> List[DltResource]:
    ...
    Returns:
        List[DltResource]: List of data_load_tool resources.
    ...
    Example:
        github_source = rest_api_resources({
            "client": {
                ...
                "auth": {
                    "token": data_load_tool.secrets["token"],
                },
            },
            "resource_defaults": {
                "primary_key": "id",
...
def create_resources(
    ...
    for resource_name in dependency_graph.static_order():
        ...
        if resolved_params is None:
            ...
            resources[resource_name] = data_load_tool.resource(
                paginate_resource,
            ...
            resources[resource_name] = data_load_tool.resource(  # type: ignore[call-overload]
