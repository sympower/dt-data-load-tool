...
def load_select_tables_from_database() -> None:
    ...
    """
    # Create a pipeline
    pipeline = data_load_tool.pipeline(pipeline_name="rfam", destination="duckdb", dataset_name="rfam_data")

    # Credentials for the sample database.
    # Note: It is recommended to configure credentials in `.data_load_tool/secrets.toml` under `sources.sql_database.credentials`
    ...
    source_1.family.apply_hints(incremental=data_load_tool.sources.incremental("updated"))
    source_1.clan.apply_hints(incremental=data_load_tool.sources.incremental("updated"))
    ...
    source_3.genome.apply_hints(incremental=data_load_tool.sources.incremental("created"))
...
def load_entire_database() -> None:
    ...
    # The database credentials are sourced from the `.data_load_tool/secrets.toml` configuration
    source = sql_database()
...
def select_with_end_value_and_row_order() -> None:
    ...
    family = sql_table(
        credentials="mysql+pymysql://rfamro@mysql-rfam-public.ebi.ac.uk:4497/Rfam",
        table="family",
        incremental=data_load_tool.sources.incremental(  # declares desc row order
...
def my_sql_via_pyarrow() -> None:
    ...
    # data_load_tool.config["normalize.parquet_normalizer.add_dlt_load_id"] = True
...
def create_unsw_flow() -> None:
    """Uploads UNSW_Flow dataset to postgres via csv stream skipping data_load_tool normalizer.
...
def specify_columns_to_load() -> None:
    ...
    # Columns can be specified per table in env var (json array) or in `.data_load_tool/config.toml`
