...
from data_load_tool.common.libs.sql_alchemy import Table, Column, Row, sqltypes, Select, TypeEngine
from data_load_tool.common import logger
from data_load_tool.common.schema.typing import TColumnSchema, TTableSchemaColumns, TTableReference

ReflectionLevel = Literal["minimal", "full", "full_with_precision"]


# optionally create generics with any so they can be imported by data_load_tool importer
if TYPE_CHECKING:
...
def sqla_col_to_column_schema(
    ...
    """Infer data_load_tool schema column type from an sqlalchemy type.
    ...
    if hasattr(sqltypes, "Uuid") and isinstance(sql_t, sqltypes.Uuid):
        ...
        logger.warning(
            ...
            " mapped to `data_load_tool` data type. When using sqlalchemy backend such data will be passed to"
...
def table_to_columns(
    ...
    """Convert an sqlalchemy table to a data_load_tool table schema."""
    return {
        col["name"]: col
        for col in (
