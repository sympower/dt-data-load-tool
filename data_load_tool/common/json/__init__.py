...
from data_load_tool.common.arithmetics import Decimal
...
def _datetime_decoder(obj: str) -> datetime:
    if obj.endswith("Z"):
        # Backwards compatibility for data encoded with previous data_load_tool version
...
if os.environ.get(known_env.DLT_USE_JSON) == "simplejson":
    from data_load_tool.common.json import _simplejson as _json_d

    json = _json_d  # type: ignore[assignment]
else:
    try:
        from data_load_tool.common.json import _orjson as _json_or

        json = _json_or  # type: ignore[assignment]
    except ImportError:
        from data_load_tool.common.json import _simplejson as _json_simple
