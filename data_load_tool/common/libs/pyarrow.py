...
from data_load_tool import version
from data_load_tool.common.exceptions import MissingDependencyException, DltException
from data_load_tool.common.schema.typing import C_DLT_ID, C_DLT_LOAD_ID, TColumnSchema, TTableSchemaColumns
from data_load_tool.common import logger
from data_load_tool.common.json import json, custom_encode, map_nested_in_place
...
from data_load_tool.common.schema.utils import is_nullable_column
from data_load_tool.common.typing import StrStr, TFileOrPath, TDataItems
...
try:
    ...
    raise MissingDependencyException(
        "data_load_tool pyarrow helpers",
...
class UnsupportedArrowTypeException(DltException):
    ...
    def generate_message(arrow_type: pyarrow.DataType, field_name: str, table_name: str) -> str:
        ...
        msg += (
            " is unsupported by data_load_tool. See documentation:"
...
class PyToArrowConversionException(DltException):
    ...
    def generate_message(
        ...
        msg += f" with data_load_tool hint `{data_type=:}` and `{inferred_arrow_type=:}`"
        msg += " " + details
        return msg
...
def should_normalize_arrow_schema(
    ...
    # remove all columns that are data_load_tool columns but are not present in arrow schema. we do not want to add such columns
...
def get_normalized_arrow_fields_mapping(schema: pyarrow.Schema, naming: NamingConvention) -> StrStr:
    ...
    # use normalize_path to be compatible with how regular columns are normalized in data_load_tool.Schema
...
def convert_numpy_to_arrow(
    ...
    Args:
        ...
        columns: data_load_tool hints about the table columns (e.g., data type, nullabe)
    ...
    # base case (0): allow pyarrow to infer type, or create array of data_load_tool specified type
    try:
    ...
    # case 1: pyarrow infers the type (e.g., float, string) THEN cast it to the data_load_tool specified type; less constraints than the base case
        ...
        for value in column_data:
            ...
            try:
                ...
                raise PyToArrowConversionException(
                    data_type=dlt_data_type,
                    inferred_arrow_type=inferred_arrow_type,
                    details="data_load_tool failed to a JSON-serializable type.",
                ) from e
            ...
            for value in column_data:
                ...
                try:
                    ...
                    raise PyToArrowConversionException(
                        data_type=dlt_data_type,
                        inferred_arrow_type=inferred_arrow_type,
                        details="data_load_tool failed to encode values to a Arrow compatible type.",
                    ) from e
    ...
    if arrow_array is None:
        raise PyToArrowConversionException(
            data_type=dlt_data_type,
            inferred_arrow_type=inferred_arrow_type,
            details="This data type seems currently unsupported by data_load_tool. Please open a GitHub issue",
        )
...
def row_tuples_to_arrow(
    ...
    # this only removes empty columns that don't have an explicit data_load_tool `data_type`
