...
from data_load_tool import version, Pipeline
from data_load_tool.common import logger
from data_load_tool.common.libs.pyarrow import pyarrow as pa
from data_load_tool.common.libs.pyarrow import cast_arrow_schema_types
from data_load_tool.common.schema.typing import TWriteDisposition, TTableSchema
from data_load_tool.common.schema.utils import get_first_column_name_with_prop, get_columns_names_with_prop
...
from data_load_tool.common.utils import assert_min_pkg_version
from data_load_tool.common.configuration.specs.mixins import WithObjectStoreRsCredentials
...
try:
    ...
    raise MissingDependencyException(
        "data_load_tool deltalake helpers",
        [f"{version.DLT_PKG_NAME}[deltalake]"],
        "Install `deltalake` so data_load_tool can create Delta tables in the `filesystem` destination.",
    )
...
def get_delta_write_mode(write_disposition: TWriteDisposition) -> str:
    """Translates data_load_tool write disposition to Delta write mode."""
...
def _deltalake_storage_options(config: FilesystemConfiguration) -> Dict[str, str]:
    ...
    if len(shared_keys) > 0:
        logger.warning(
            ...
            "keys also provided by data_load_tool's credential system: "
            + ", ".join([f"`{key}`" for key in shared_keys])
            + ". data_load_tool will use the values in `deltalake_storage_options`."
        )
