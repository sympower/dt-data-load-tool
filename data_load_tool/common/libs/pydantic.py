...
from data_load_tool.common.schema.typing import TSchemaEvolutionMode, TTableSchemaColumns
...
try:
    ...
    raise MissingDependencyException(
        "data_load_tool Pydantic helpers", ["pydantic"], "Both Pydantic 1.x and 2.x are supported"
    )
...
class DltConfig(TypedDict, total=False):
    """data_load_tool configuration that can be attached to Pydantic model

    Example below removes `nested` field from the resulting data_load_tool schema.
    ...
    """If True, columns of complex types (`dict`, `list`, `BaseModel`) will be excluded from data_load_tool schema generated from the model"""
...
def apply_schema_contract_to_model(
    ...
    # pass data_load_tool config along
