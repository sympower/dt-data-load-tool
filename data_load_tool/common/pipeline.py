...
from data_load_tool.common.configuration.specs import ContainerInjectableContext
...
from data_load_tool.common.destination import TDestinationReferenceArg, AnyDestination
from data_load_tool.common.destination.exceptions import DestinationHasFailedJobs
from data_load_tool.common.exceptions import (
    PipelineStateNotAvailable,
    SourceSectionNotAvailable,
    ResourceNameNotAvailable,
)
from data_load_tool.common.metrics import (
    DataWriterMetrics,
    ExtractDataInfo,
    ExtractMetrics,
    LoadMetrics,
...
from data_load_tool.common.storages.load_storage import LoadPackageInfo
from data_load_tool.common.time import ensure_pendulum_datetime, precise_time
from data_load_tool.common.typing import DictStrAny, REPattern, StrAny, SupportsHumanize, TColumnNames
from data_load_tool.common.jsonpath import delete_matches, TAnyJsonPath
from data_load_tool.common.data_writers.writers import TLoaderFileFormat
from data_load_tool.common.utils import RowCounts, merge_row_counts
from data_load_tool.common.versioned_state import TVersionedState
...
class ExtractInfo(StepInfo[ExtractMetrics], _ExtractInfo):  # type: ignore[misc]
    ...
    def asdict(self) -> DictStrAny:
        """A dictionary representation of ExtractInfo that can be loaded with `data_load_tool`"""
        d = super().asdict()
...
class NormalizeInfo(StepInfo[NormalizeMetrics], _NormalizeInfo):  # type: ignore[misc]
    ...
    def asdict(self) -> DictStrAny:
        """A dictionary representation of NormalizeInfo that can be loaded with `data_load_tool`"""
        d = super().asdict()
        # transform metrics
        d.pop("metrics")
        load_metrics: Dict[str, List[Any]] = {
...
class LoadInfo(StepInfo[LoadMetrics], _LoadInfo):  # type: ignore[misc]
    ...
    def asdict(self) -> DictStrAny:
        """A dictionary representation of LoadInfo that can be loaded with `data_load_tool`"""
        d = super().asdict()
        # transform metrics
        d.pop("metrics")
    ...
    def pipeline(self) -> SupportsPipeline:
        ...
        if not self._pipeline:
            ...
            assert PipelineContext._DEFERRED_PIPELINE is not None, (
                ...
                " calling data_load_tool.pipeline() from another thread?"
            )
...
def source_state() -> DictStrAny:
    ...
    #### Note:
    The source state is a python dictionary-like object that is available within the `@data_load_tool.source` and `@data_load_tool.resource` decorated functions and may be read and written to.
    ...
    * Any JSON-serializable values can be written and the read from the state. `data_load_tool` dumps and restores instances of Python bytes, DateTime, Date and Decimal types.
...
def resource_state(
    ...
    Note that this function accepts the resource name as optional argument. There are rare cases when `data_load_tool` is not able to resolve resource name due to requesting function
    ...
    Summary:
        The resource state is a python dictionary-like object that is available within the `@data_load_tool.resource` decorated functions and may be read and written to.
        The data within the state is loaded into destination together with any other extracted data and made automatically available to the source/resource extractor functions when they are run next time.
        When using the state:
...
def get_dlt_pipelines_dir() -> str:
    ...
    1. in user home directory ~/.data_load_tool/pipelines/
    2. if current user is root in /var/data_load_tool/pipelines
    3. if current user does not have a home directory in /tmp/data_load_tool/pipelines
    """
...
def get_current_pipe_name() -> str:
    """When executed from withing data_load_tool.resource decorated function, gets pipe name associated with current thread.
