...
from data_load_tool.common import logger as dlt_logger
...
class LogCollector(Collector):
    ...
    def __init__(
        ...
        """
        Collector writing to a `logger` every `log_period` seconds. The logger can be a Python logger instance, text stream, or None that will attach `data_load_tool` logger
    ...
    def dump_counters(self) -> None:
        ...
        log_message = "\n".join(log_lines)
        if not self.logger:
            # try to attach data_load_tool logger
