...
from data_load_tool.common.configuration.exceptions import ConfigFieldMissingException, LookupTrace
from data_load_tool.common.configuration.providers.provider import ConfigProvider
from data_load_tool.common.configuration.specs import BaseConfiguration, is_base_configuration_inner_hint
from data_load_tool.common.configuration.utils import deserialize_value, log_traces, auto_cast
...
class _Accessor(abc.ABC):
    ...
    def _get_value(self, field: str, type_hint: Type[Any] = None) -> Tuple[Any, List[LookupTrace]]:
        # get default hint type, in case of data_load_tool.secrets it it TSecretValue
...
class _ConfigAccessor(_Accessor):
    ...
    "A placeholder that tells data_load_tool to replace it with actual config value during the call to a source or resource decorated function."
...
class _SecretsAccessor(_Accessor):
    ...
    "A placeholder that tells data_load_tool to replace it with actual secret during the call to a source or resource decorated function."


config = _ConfigAccessor()
"""Dictionary-like access to all config values to data_load_tool"""

secrets = _SecretsAccessor()
"""Dictionary-like access to all secrets known known to data_load_tool"""
