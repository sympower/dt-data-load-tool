...
class SettingsTomlProvider(CustomLoaderDocProvider):
    ...
    def __init__(
        ...
        it will additionally look for `file_name` in `data_load_tool` global dir (home dir by default) and merge the content.
    ...
    def _read_google_colab_secrets(self, name: str, file_name: str) -> tomlkit.TOMLDocument:
        ...
        try:
            ...
            # when data_load_tool cli is run, userdata is available but without a kernel
            if not is_notebook():
                return None
