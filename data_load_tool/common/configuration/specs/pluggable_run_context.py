...
from data_load_tool.common.configuration.specs.base_configuration import ContainerInjectableContext
from data_load_tool.common.configuration.specs.runtime_configuration import RuntimeConfiguration
...
class SupportsRunContext(Protocol):
    """Describes where `data_load_tool` looks for settings, pipeline working folder. Implementations must be picklable."""
    ...
    def name(self) -> str:
        ...
        is active, will be scoped to it. Typically corresponds to Python package name ie. `data_load_tool`.
        """

    @property
    def global_dir(self) -> str:
        """Directory in which global settings are stored ie ~/.data_load_tool/"""
...
class PluggableRunContext(ContainerInjectableContext):
    ...
    def initialize_runtime(self, runtime_config: RuntimeConfiguration) -> None:
        ...
        from data_load_tool.common.runtime.init import initialize_runtime
