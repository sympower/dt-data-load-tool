...
from data_load_tool.common.known_env import DLT_DISABLE_PLUGINS

hookspec = pluggy.HookspecMarker("data_load_tool")
hookimpl = pluggy.HookimplMarker("data_load_tool")
...
class PluginContext(ContainerInjectableContext):
    ...
    def __init__(self) -> None:
        super().__init__()
        self.manager = pluggy.PluginManager("data_load_tool")
        self.plugin_modules = []
        ...
        # cli
        from data_load_tool.cli import plugins
...
def load_setuptools_entrypoints(m: pluggy.PluginManager) -> List[str]:
    ...
    loads entry points in group `data_load_tool` and instantiates them to initialize plugins.
