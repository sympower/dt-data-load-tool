...
from data_load_tool.common.utils import extend_list_deduplicated
...
from data_load_tool.common.normalizers import TNormalizersConfig, NamingConvention
from data_load_tool.common.normalizers.json import DataItemNormalizer, TNormalizedRowIterator
from data_load_tool.common.schema import utils
from data_load_tool.common.data_types import py_type_to_sc_type, coerce_value, TDataType
...
from data_load_tool.common.schema.normalizers import import_normalizers, configured_normalizers
...
class Schema:
    ...
    """Normalized name of the data_load_tool state table"""
    ...
    def apply_schema_contract(
        ...
        * discard_value: allow no schema change and filter out the value but load the rest of the row
        ...
        for column_name, column in list(partial_table["columns"].items()):
            # data_load_tool cols may always be added
    ...
    def data_tables(
        ...
        """Gets list of all tables, that hold the loaded data. Excludes data_load_tool tables. Excludes incomplete tables (ie. without columns)"""
        return [
            t
    ...
    def data_table_names(
        ...
        """Returns list of table names. Excludes data_load_tool table names."""
        return [
            t["name"]
    ...
    def _replace_and_apply_naming(
        ...
        # do a sanity check - data_load_tool tables must start with data_load_tool prefix
