...
from data_load_tool.common.pendulum import timedelta, pendulum
...
"""A special callable Any that returns argument but is recognized as Any type by data_load_tool hint checkers"""
NoneType = type(None)
...
class ConfigValueSentinel(NamedTuple):
    ...
    def __repr__(self) -> str:
        if self.default_literal == "data_load_tool.config.value":
            inst_ = "ConfigValue"
        else:
            inst_ = "SecretValue"
...
ConfigValue: None = ConfigValueSentinel("data_load_tool.config.value", AnyType)  # type: ignore[assignment]
"""Config value indicating argument that may be injected by config provider. Evaluates to None when type checking"""

SecretValue: None = ConfigValueSentinel("data_load_tool.secrets.value", TSecretValue)  # type: ignore[assignment]
...
class SupportsHumanize(Protocol):
    def asdict(self) -> DictStrAny:
        """Represents object as dict with a schema loadable by data_load_tool"""
        ...
