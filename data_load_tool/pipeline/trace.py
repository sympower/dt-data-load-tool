...
from data_load_tool.common.configuration import is_secret_hint
...
from data_load_tool.common.configuration.utils import _RESOLVED_TRACES, ResolvedValueTrace
from data_load_tool.common.configuration.container import Container
from data_load_tool.common.exceptions import ExceptionTrace, ResourceNameNotAvailable
from data_load_tool.common.logger import suppress_and_warn
from data_load_tool.common.runtime.exec_info import TExecutionContext, get_execution_context
...
from data_load_tool.common.pipeline import get_current_pipe_name
from data_load_tool.common.storages.file_storage import FileStorage
from data_load_tool.common.typing import DictStrAny, StrAny, SupportsHumanize
from data_load_tool.common.utils import uniq_id, get_exception_trace_chain
...
class PipelineStepTrace(SupportsHumanize, _PipelineStepTrace):
    ...
    def asdict(self) -> DictStrAny:
        """A dictionary representation of PipelineStepTrace that can be loaded with `data_load_tool`"""
        d = self._asdict()
        if self.step_info:
...
class PipelineTrace(SupportsHumanize, _PipelineTrace):
    ...
    def asdict(self) -> DictStrAny:
        """A dictionary representation of PipelineTrace that can be loaded with `data_load_tool`"""
        d = self._asdict()
...
def get_exception_traces(exc: BaseException, container: Container = None) -> List[ExceptionTrace]:
    ...
    for trace in traces:
        # only to data_load_tool exceptions
