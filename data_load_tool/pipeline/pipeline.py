...
from data_load_tool.common.configuration import inject_section, known_sections
...
from data_load_tool.common.runtime import signals, apply_runtime_config
...
from data_load_tool.common.schema.utils import normalize_schema_name
from data_load_tool.common.storages.exceptions import LoadPackageNotFound
from data_load_tool.common.typing import ConfigValue, TFun, TSecretStrValue, TColumnNames
...
from data_load_tool.common.destination import (
    Destination,
...
from data_load_tool.common.pipeline import (
    ExtractInfo,
    LoadInfo,
    NormalizeInfo,
    PipelineContext,
...
from data_load_tool.common.utils import is_interactive
from data_load_tool.common.warnings import deprecated, Dlt04DeprecationWarning
from data_load_tool.common.versioned_state import json_encode_state, json_decode_state

from data_load_tool.destinations.configuration import WithLocalFiles
from data_load_tool.extract import DltSource
from data_load_tool.extract.exceptions import SourceExhausted
from data_load_tool.extract.extract import Extract, data_to_sources
from data_load_tool.normalize import Normalize
from data_load_tool.normalize.configuration import NormalizeConfiguration
from data_load_tool.destinations.sql_client import SqlClientBase, WithSqlClient
...
from data_load_tool.destinations.dataset import (
    dataset,
...
from data_load_tool.pipeline.configuration import PipelineConfiguration
from data_load_tool.pipeline.progress import _Collector, _NULL_COLLECTOR
...
from data_load_tool.pipeline.typing import TPipelineStep
...
from data_load_tool.common.storages.load_package import TLoadPackageState
from data_load_tool.pipeline.helpers import refresh_source
...
class Pipeline(SupportsPipeline):
    ...
    is_active: bool
    """Tells if instance is currently active and available via data_load_tool.pipeline()"""
    ...
    def __init__(
        ...
        """Initializes the Pipeline class which implements `data_load_tool` pipeline. Please use `pipeline` function in `data_load_tool` module to create a new Pipeline instance."""
    ...
    def run(
        ...
        * a `list` or `Iterable` of any JSON-serializable objects ie. `data_load_tool.run([1, 2, 3], table_name="numbers")`
        * any `Iterator` or a function that yield (`Generator`) ie. `data_load_tool.run(range(1, 10), table_name="range")`
        * a function or a list of functions decorated with @data_load_tool.resource ie. `data_load_tool.run([chess_players(title="GM"), chess_games()])`
        * a function or a list of functions decorated with @data_load_tool.source.

        Please note that `data_load_tool` deals with `bytes`, `datetime`, `decimal` and `uuid` objects so you are free to load documents containing ie. binary data or dates.
        ...
        Args:
            ...
            destination (str | DestinationReference, optional): A name of the destination to which data_load_tool will load the data, or a destination module imported from `data_load_tool.destination`.
                If not provided, the value passed to `data_load_tool.pipeline` will be used.
            ...
            dataset_name (str, optional): A name of the dataset to which the data will be loaded. A dataset is a logical group of tables ie. `schema` in relational databases or folder grouping many files.
                If not provided, the value passed to `data_load_tool.pipeline` will be used. If not provided at all then defaults to the `pipeline_name`
            ...
            credentials (Any, optional): Credentials for the `destination` ie. database connection string or a dictionary with google cloud credentials.
                In most cases should be set to None, which lets `data_load_tool` to use `secrets.toml` or environment variables to infer right credentials values.
            ...
            table_name (str, optional): The name of the table to which the data should be loaded within the `dataset`. This argument is required for a `data` that is a list/Iterable or Iterator without `__name__` attribute.
                ...
                * `@data_load_tool.resource` - resource contains the full table schema and that includes the table name. `table_name` will override this property. Use with care!
                * `@data_load_tool.source` - source contains several resources each with a table schema. `table_name` will override all table names within the source and load the data into single table.
            ...
            write_disposition (TWriteDispositionConfig, optional): Controls how to write data to a table. Accepts a shorthand string literal or configuration dictionary.
                ...
                Please note that in case of `data_load_tool.resource` the table schema value will be overwritten and in case of `data_load_tool.source`, the values in all resources will be overwritten.
            ...
            schema (Schema, optional): An explicit `Schema` object in which all table schemas will be grouped. By default `data_load_tool` takes the schema from the source (if passed in `data` argument) or creates a default one itself.
        ...
        Returns:
            LoadInfo: Information on loaded data including the list of package ids and failed job statuses. Please not that `data_load_tool` will not raise if a single job terminally fails. Such information is provided via LoadInfo.
        """
    ...
    def activate(self) -> None:
        ...
        The active pipeline is used as a context for several `data_load_tool` components. It provides state to sources and resources evaluated outside of
        `pipeline.run` and `pipeline.extract` method. For example, if the source you use is accessing state in `data_load_tool.source` decorated function, the state is provided
        by active pipeline.
        ...
        with name `chess_pipeline` is active and `data_load_tool` looks for `BigQuery` configuration, it will look in `chess_pipeline.destination.bigquery.credentials` first and then in
        ...
        Pipeline created or attached with `data_load_tool.pipeline`/'data_load_tool.attach` is automatically activated. `run`, `load` and `extract` methods also activate pipeline.
        """
    ...
    def _get_destination_clients(
        ...
        if not self._destination:
            raise PipelineConfigMissing(
                ...
                " directly or via .data_load_tool config.toml file or environment variable.",
            )
        ...
        if isinstance(destination_client.config, DestinationClientStagingConfiguration):
            if not self.dataset_name and self.dev_mode:
                logger.warning(
                    ...
                    " dataset_name argument in data_load_tool.pipeline or run method"
                )
    ...
    def _get_state(self) -> TPipelineState:
        try:
            ...
            if "initial_cwd" not in _local:
                _local["initial_cwd"] = os.path.abspath(data_load_tool.current.run_context().local_dir)
