...
from data_load_tool.common.exceptions import PipelineException
from data_load_tool.common.pipeline import StepInfo, StepMetrics, SupportsPipeline
...
class PipelineStateEngineNoUpgradePathException(PipelineException):
    def __init__(
        ...
        super().__init__(
            ...
            f" {to_engine}, stopped at {from_engine}. You possibly tried to run an older data_load_tool"
            " version against a destination you have previously loaded data to with a newer data_load_tool"
            " version.",
        )
...
class PipelineHasPendingDataException(PipelineException):
    def __init__(self, pipeline_name: str, pipelines_dir: str) -> None:
        msg = (
            ...
            f" {pipelines_dir} contains pending extracted files or load packages. Use `data_load_tool pipeline"
...
class PipelineNeverRan(PipelineException):
    def __init__(self, pipeline_name: str, pipelines_dir: str) -> None:
        msg = (
            f" Operation failed because pipeline with name {pipeline_name} in working directory"
            f" {pipelines_dir} was never run or was never synced with destination. Use `data_load_tool"
