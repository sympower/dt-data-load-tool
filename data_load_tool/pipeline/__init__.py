...
from data_load_tool.common.configuration.specs import known_sections
...
from data_load_tool.common.typing import TSecretStrValue, Any
from data_load_tool.common.configuration import with_config
from data_load_tool.common.configuration.container import Container
from data_load_tool.common.configuration.inject import get_orig_args, last_config
from data_load_tool.common.destination import TLoaderFileFormat, Destination, TDestinationReferenceArg
from data_load_tool.common.pipeline import LoadInfo, PipelineContext, get_dlt_pipelines_dir, TRefreshMode
from data_load_tool.common.runtime import apply_runtime_config, init_telemetry

from data_load_tool.pipeline.configuration import PipelineConfiguration, ensure_correct_pipeline_kwargs
from data_load_tool.pipeline.pipeline import Pipeline
from data_load_tool.pipeline.progress import _from_name as collector_from_name, TCollectorArg, _NULL_COLLECTOR
from data_load_tool.pipeline.warnings import full_refresh_argument_deprecated
...
def pipeline(
    ...
    """Creates a new instance of `data_load_tool` pipeline, which moves the data from the source ie. a REST API to a destination ie. database or a data lake.
    ...
    Args:
        ...
        pipelines_dir (str, optional): A working directory in which pipeline state and temporary files will be stored. Defaults to user home directory: `~/data_load_tool/pipelines/`.
        ...
        staging (str | DestinationReference, optional): A name of the destination where data_load_tool will stage the data before final loading, or a destination module imported from `data_load_tool.destination`.
            May also be provided to `run` method of the `pipeline`.
        ...
        progress(str, Collector): A progress monitor that shows progress bars, console or log messages with current information on sources, resources, data items etc. processed in
            `extract`, `normalize` and `load` stage. Pass a string with a collector name or configure your own by choosing from `data_load_tool.progress` module.
...
def run(
    ...
    * a function or a list of functions decorated with @data_load_tool.source.
    ...
    Please note that `data_load_tool` deals with `bytes`, `datetime`, `decimal` and `uuid` objects so you are free to load binary data or documents containing dates.
...
from data_load_tool.pipeline import trace, track, platform
