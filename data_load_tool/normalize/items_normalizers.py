...
from data_load_tool.common.data_writers.writers import ArrowToObjectAdapter
from data_load_tool.common.json import custom_pua_decode, may_have_pua
...
from data_load_tool.common.schema.utils import dlt_id_column, has_table_seen_data
...
from data_load_tool.common.typing import DictStrAny, TDataItem
from data_load_tool.common.schema import TSchemaUpdate, Schema
from data_load_tool.common.exceptions import MissingDependencyException
from data_load_tool.common.normalizers.utils import generate_dlt_ids
...
class ArrowItemsNormalizer(ItemsNormalizer):
    ...
    def _fix_schema_precisions(
        ...
        for key, column in table["columns"].items():
            if column.get("data_type") in ("timestamp", "time"):
                ...
                if prec is not None:
                    # apply the arrow schema precision to data_load_tool column schema
                    try:
    ...
    def __call__(self, extracted_items_file: str, root_table_name: str) -> List[TSchemaUpdate]:
        ...
        from data_load_tool.common.libs.pyarrow import get_parquet_metadata
        ...
        # and apply them to data_load_tool schema
    ...
    def __call__(self, extracted_items_file: str, root_table_name: str) -> List[TSchemaUpdate]:
        ...
        if not completed_columns:
            logger.warning(
                ...
                " resource or with data_load_tool.mark.with_hints or create the destination table yourself."
            )
        with self.normalize_storage.extracted_packages.storage.open_file(
            extracted_items_file, "rb"
        ) as f:
