...
from data_load_tool.common.jsonpath import compile_path, extract_simple_field_name
...
from data_load_tool.common.configuration import configspec, ConfigurationValueError
...
from data_load_tool.common.data_types.type_helpers import (
    coerce_from_date_types,
    coerce_value,
    py_type_to_sc_type,
)

from data_load_tool.extract.exceptions import IncrementalUnboundError
from data_load_tool.extract.incremental.exceptions import (
    IncrementalCursorPathMissing,
    IncrementalPrimaryKeyMissing,
)
from data_load_tool.common.incremental.typing import (
...
from data_load_tool.extract.items import SupportsPipe, TTableHintTemplate
from data_load_tool.extract.items_transform import ItemTransform
from data_load_tool.extract.incremental.transform import (
    JsonIncremental,
    ArrowIncremental,
...
try:
    from data_load_tool.common.libs.pyarrow import is_arrow_item
except MissingDependencyException:
    is_arrow_item = lambda item: False

try:
    from data_load_tool.common.libs.pandas import pandas
except MissingDependencyException:
    pandas = None
...
class Incremental(ItemTransform[TDataItem], BaseConfiguration, Generic[TCursorValue]):
    ...
    >>> def some_data(created_at=data_load_tool.sources.incremental('created_at', '2023-01-01T00:00:00Z'):
    ...
    >>> def some_data():
    >>>     last_value = data_load_tool.sources.incremental.from_existing_state("some_data", "item.ts")
    >>>     ...
    >>>
    >>> r = some_data().add_step(data_load_tool.sources.incremental("item.ts", initial_value=now, primary_key="delta"))
    ...
    Args:
        ...
        allow_external_schedulers: If set to True, allows data_load_tool to look for external schedulers from which it will take "initial_value" and "end_value" resulting in loading only
    ...
    def _cursor_datetime_check(value: Any, arg_name: str) -> None:
        if value and isinstance(value, datetime) and value.tzinfo is None:
            logger.warning(
                ...
                " in an error when such values  are compared by Incremental class. Note that `data_load_tool`"
    ...
    def _join_external_scheduler(self) -> None:
        ...
        def _ensure_airflow_end_date(
            ...
            """if end_date is in the future or same as start date (manual run), set it to None so data_load_tool state is used for incremental loading"""
            now = pendulum.now()
...
class IncrementalResourceWrapper(ItemTransform[TDataItem]):
    ...
    def __init__(self, primary_key: Optional[TTableHintTemplate[TColumnNames]] = None) -> None:
        ...
        The wrapper delays instantiation of the Incremental to the moment of actual execution and is currently used by `data_load_tool.resource` decorator.
