...
from data_load_tool.common.configuration import known_sections, resolve_configuration, with_config
from data_load_tool.common import logger
from data_load_tool.common.configuration.specs import BaseConfiguration, configspec
...
from data_load_tool.common.runtime.collector import Collector, NULL_COLLECTOR
from data_load_tool.common.typing import TDataItems, TDataItem, TLoaderFileFormat
...
from data_load_tool.extract.hints import HintsMeta, TResourceHints
from data_load_tool.extract.resource import DltResource
from data_load_tool.extract.items import DataItemWithMeta, TableNameMeta
from data_load_tool.extract.storage import ExtractorItemStorage
from data_load_tool.normalize.configuration import ItemsNormalizerConfiguration

try:
    from data_load_tool.common.libs import pyarrow
    from data_load_tool.common.libs.pyarrow import pyarrow as pa, TAnyArrowItem, UnsupportedArrowTypeException
except MissingDependencyException:
    pyarrow = None
    pa = None

try:
    from data_load_tool.common.libs.pandas import pandas, pandas_to_arrow
except MissingDependencyException:
    pandas = None
...
def with_file_import(
    ...
    If number of records in `file_path` is known, pass it in `items_count` so `data_load_tool` can generate correct extract metrics.

    Note that `data_load_tool` does not sniff schemas from data and will not guess right file format for you.
    """
    metrics = DataWriterMetrics(file_path, items_count, 0, 0, 0)
    item: TDataItem = None
    # if hints are dict assume that this is data_load_tool schema, if not - that it is arrow table
...
class ArrowExtractor(Extractor):
    ...
    Compares the arrow schema to actual data_load_tool table schema to reorder the columns and to
    ...
    def _compute_table(
        ...
        for item in reversed(items):
            ...
            if override_warn:
                logger.warning(
                    f"In resource: {resource.name}, when merging arrow schema with data_load_tool schema,"
                    " several column hints were different. data_load_tool schema hints were kept and arrow"
