...
from data_load_tool.common.configuration import with_config, get_fun_spec, known_sections, configspec
from data_load_tool.common.configuration.container import Container
from data_load_tool.common.configuration.exceptions import ContextDefaultCannotBeCreated
from data_load_tool.common.configuration.inject import set_fun_spec
...
from data_load_tool.common.exceptions import ArgumentsOverloadException
from data_load_tool.common.pipeline import PipelineContext
from data_load_tool.common.reflection.spec import spec_from_signature
from data_load_tool.common.schema.utils import DEFAULT_WRITE_DISPOSITION
...
from data_load_tool.common.typing import AnyFun, ParamSpec, Concatenate, TDataItem, TDataItems, TColumnNames
...
from data_load_tool.extract.utils import simulate_func_call
...
from data_load_tool.extract.items import TTableHintTemplate
from data_load_tool.extract.source import DltSource
from data_load_tool.extract.reference import SourceReference, SourceFactory, TDltSourceImpl, TSourceFunParams
from data_load_tool.extract.resource import DltResource, TUnboundDltResource, TDltResourceImpl
from data_load_tool.extract.incremental import TIncrementalConfig


@configspec
class SourceSchemaInjectableContext(ContainerInjectableContext):
    """A context containing the source schema, present when data_load_tool.source/resource decorated function is executed"""
...
class SourceInjectableContext(ContainerInjectableContext):
    """A context containing the source schema, present when data_load_tool.resource decorated function is executed"""
...
def source(
    ...
    """A decorator that transforms a function returning one or more `data_load_tool resources` into a `data_load_tool source` in order to load it with `data_load_tool`.

    Note:
    A `data_load_tool source` is a logical grouping of resources that are often extracted and loaded together. A source is associated with a schema, which describes the structure of the loaded data and provides instructions how to load it.
    ...
    >>> def chess(username, chess_url: str = data_load_tool.config.value, api_secret = data_load_tool.secrets.value, title: str = "GM"):
    ...
    Here `username` is a required, explicit python argument, `chess_url` is a required argument, that if not explicitly passed will be taken from configuration ie. `config.toml`, `api_secret` is a required argument, that if not explicitly passed will be taken from data_load_tool secrets ie. `secrets.toml`.
    See https://dlthub.com/docs/general-usage/credentials/ for details.

    Args:
        func: A function that returns a data_load_tool resource or a list of those or a list of any data items that can be loaded by `data_load_tool`.
        ...
        schema (Schema, optional): An explicit `Schema` instance to be associated with the source. If not present, `data_load_tool` creates a new `Schema` object with provided `name`. If such `Schema` already exists in the same folder as the module containing the decorated function, such schema will be loaded from file.
...
def resource(
    ...
    """When used as a decorator, transforms any generator (yielding) function into a `data_load_tool resource`. When used as a function, it transforms data in `data` argument into a `data_load_tool resource`.
    ...
    A `data_load_tool resource` is python representation of a `resource` that combines both data and metadata (table schema) that describes the structure and instructs the loading of the data.
    A `data_load_tool resource` is also an `Iterable` and can used like any other iterable object ie. list or tuple.
    ...
    >>> def user_games(username, chess_url: str = data_load_tool.config.value, api_secret = data_load_tool.secrets.value):
    ...
    Args:
        data (Callable | Any, optional): a function to be decorated or a data compatible with `data_load_tool` `run`.
        ...
        selected (bool, optional): When `True` `data_load_tool pipeline` will extract and load this resource, if `False`, the resource will be ignored.
    ...
    Raises:
        ...
        InvalidResourceDataType: indicates that the `data` argument cannot be converted into `data_load_tool resource`
    ...
    def make_resource(_name: str, _section: str, _data: Any) -> TDltResourceImpl:
        ...
        # later in normalizer data_load_tool/common/normalizers/json/relational.py
    ...
    def wrap_standalone(
        ...
        if not standalone:
            # we return a DltResource that is callable and returns data_load_tool resource when called
...
def transformer(
    ...
    """A form of `data_load_tool resource` that takes input from other resources via `data_from` argument in order to enrich or transform the data.

    The decorated function `f` must take at least one argument of type TDataItems (a single item or list of items depending on the resource `data_from`). `data_load_tool` will pass
    ...
    Example:
    >>> @data_load_tool.resource
    >>> def players(title, chess_url=data_load_tool.config.value):
    >>>     r = requests.get(f"{chess_url}titled/{title}")
    >>>     yield r.json()["players"]  # returns list of player names
    >>>
    >>> # this resource takes data from players and returns profiles
    >>> @data_load_tool.transformer(write_disposition="replace")
...
def get_source_schema() -> Schema:
    """When executed from the function decorated with @data_load_tool.source, returns a writable source Schema"""
    try:
...
def get_source() -> DltSource:
    """When executed from the function decorated with @data_load_tool.resource, returns currently extracted source"""
    try:
