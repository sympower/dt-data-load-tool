...
from data_load_tool.common.schema.typing import TColumnName, TSchemaContract
from data_load_tool.common.schema.utils import normalize_table_identifiers
from data_load_tool.common.typing import StrAny, TDataItem
...
from data_load_tool.common.utils import graph_find_scc_nodes, flatten_list_or_items, graph_edges_to_nodes

from data_load_tool.extract.items import TDecompositionStrategy
from data_load_tool.extract.pipe_iterator import ManagedPipeIterator
...
class DltSource(Iterable[TDataItem]):
    """Groups several `data_load_tool resources` under a single schema and allows to perform operations on them.

    The instance of this class is created whenever you call the `data_load_tool.source` decorated function. It automates several functions for you:
    * You can pass this instance to `data_load_tool` `run` method in order to load all data present in the `data_load_tool resources`.
    ...
    * It implements `Iterable` interface so you can get all the data from the resources yourself and without data_load_tool pipeline present.
    ...
    * You can use a `run` method to load the data with a default instance of data_load_tool pipeline.
    * You can get source read only state for the currently active Pipeline instance
    ...
    def from_data(cls, schema: Schema, section: str, data: Any) -> Self:
        """Converts any `data` supported by `data_load_tool` `run` method into `data_load_tool source` with a name `section`.`name` and `schema` schema."""
    ...
    def run(self) -> SupportsPipelineRun:
        """A convenience method that will call `run` run on the currently active `data_load_tool` pipeline. If pipeline instance is not found, one with default settings will be created."""
