...
from data_load_tool.common.reflection.utils import set_ast_parents
...
def parse_init_script(
    ...
    if len(visitor.mod_aliases) == 0:
        raise CliCommandInnerException(
            command,
            f"The pipeline script {init_script_name} does not import data_load_tool and does not seem to run"
            " any pipelines",
        )
...
def make_dlt_settings_path(path: str = None) -> str:
    """Returns path to file in data_load_tool settings folder. Returns settings folder if path not specified."""
