...
class SourceRequirements:
    ...
    """Final data_load_tool requirement that may be updated with destination extras"""
    dlt_requirement_base: Requirement
    """Original data_load_tool requirement without extras"""
    ...
    def _ensure_dlt_requirement(self) -> Requirement:
        """Find or create data_load_tool requirement"""
    ...
    def update_dlt_extras(self, destination_name: str) -> None:
        """Update the data_load_tool requirement to include destination"""
    ...
    def is_installed_dlt_compatible(self) -> bool:
        """Check whether currently installed version is compatible with data_load_tool requirement

        For example, requirements.txt of the source may specify data_load_tool>=0.3.5,<0.4.0
        and we check whether the installed data_load_tool version (e.g. 0.3.6) falls within this range.
        """
        if not self.dlt_requirement:
            return True
