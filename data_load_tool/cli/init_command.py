...
from data_load_tool.common.reflection.utils import rewrite_python_script
from data_load_tool.common.runtime import run_context
from data_load_tool.common.schema.utils import is_valid_schema_name
...
import data_load_tool.reflection.names as n
from data_load_tool.reflection.script_inspector import import_pipeline_script

from data_load_tool.cli import echo as fmt, pipeline_files as files_ops, source_detection
from data_load_tool.cli import utils
from data_load_tool.cli.config_toml_writer import WritableConfigValue, write_values
from data_load_tool.cli.pipeline_files import (
    TEMPLATE_FILES,
    SOURCES_MODULE_NAME,
...
def _clone_and_get_verified_sources_storage(repo_location: str, branch: str = None) -> FileStorage:
    ...
    # copy data_load_tool source files from here
...
def _welcome_message(
    ...
    if source_configuration.source_type in ["template", "core"]:
        ...
        fmt.echo(
            "* Review and change how data_load_tool loads your data in %s"
    ...
    if dependency_system:
        ...
        fmt.echo(
            "  If the data_load_tool dependency is already added, make sure you install the extra for %s to it"
...
def list_sources_command(repo_location: str, branch: str = None) -> None:
    fmt.echo("---")
    fmt.echo("Available data_load_tool core sources:")
    fmt.echo("---")
    ...
    fmt.echo("---")
    fmt.echo("Available data_load_tool single file templates:")
    fmt.echo("---")
    ...
    for source_name, source_configuration in _list_verified_sources(repo_location, branch).items():
        ...
        if source_name in core_sources.keys():
            msg += "(Deprecated since data_load_tool 1.0.0 in favor of core source of the same name) "
...
def init_command(
    ...
    if not source_configuration.requirements.is_installed_dlt_compatible():
        ...
        fmt.warning(msg)
        if not fmt.confirm(
            "Would you like to continue anyway? (you can update data_load_tool after this step)", default=True
        ):
            fmt.echo(
                "You can update data_load_tool with: pip3 install -U"
                f' "{source_configuration.requirements.dlt_requirement_base}"'
            )
            return
    ...
    if visitor.is_destination_imported:
        raise CliCommandInnerException(
            ...
            " from data_load_tool.destinations. You should specify destinations by name when calling"
            " data_load_tool.pipeline or data_load_tool.run in init scripts.",
        )
    if n.PIPELINE not in visitor.known_calls:
        raise CliCommandInnerException(
            "init",
            f"The pipeline script {source_configuration.src_pipeline_script} does not seem to"
            " initialize a pipeline with data_load_tool.pipeline. Please initialize pipeline explicitly in"
            " your init scripts.",
        )
    ...
    if is_new_source:
        if source_configuration.source_type == "core":
            fmt.echo(
                "Creating a new pipeline with the data_load_tool core source %s (%s)"
            ...
            if eject_source:
                ...
                fmt.echo(
                    "NOTE: Beginning with data_load_tool 1.0.0, the source %s will no longer be copied from"
                    " the verified sources repo but imported from data_load_tool.sources. You can provide the"
            ...
            if source_configuration.is_default_template:
                fmt.echo(
                    "NOTE: Could not find a data_load_tool source or template wih the name %s. Selecting the"
                    " default template." % (fmt.bold(source_name))
                )
                fmt.echo(
                    "NOTE: In case you did not want to use the default template, run 'data_load_tool init -l'"
                    " to see all available sources and templates."
                )
            fmt.echo(
                "Creating and configuring a new pipeline with the data_load_tool core template %s (%s)"
