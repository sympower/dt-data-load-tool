...
from data_load_tool.common.pipeline import resource_state, get_dlt_pipelines_dir, TSourceState
from data_load_tool.common.destination.reference import TDestinationReferenceArg
...
from data_load_tool.common.storages import FileStorage, PackageStorage
from data_load_tool.pipeline.helpers import DropCommand
...
def pipeline_command(
    ...
    **command_kwargs: Any,
    ...
    try:
        ...
        p = data_load_tool.attach(pipeline_name=pipeline_name, pipelines_dir=pipelines_dir)
    ...
    if operation == "show":
        ...
        from data_load_tool.helpers.streamlit_app import index
    ...
    if operation == "info":
        ...
        if trace is None or len(trace.steps) == 0:
            ...
            fmt.echo(
                "Pipeline has last run trace. Use 'data_load_tool pipeline %s trace' to inspect "
                % pipeline_name
            )
    ...
    if operation == "drop":
        ...
        if drop.is_empty:
            ...
            fmt.echo(f"data_load_tool pipeline -v {p.pipeline_name} info")
