...
from data_load_tool.common.configuration.exceptions import LookupTrace, ConfigFieldMissingException
from data_load_tool.common.configuration.providers import (
    CONFIG_TOML,
    EnvironProvider,
    StringTomlProvider,
)
from data_load_tool.common.git import get_origin, get_repo, Repo
from data_load_tool.common.configuration.specs.runtime_configuration import get_default_pipeline_name
from data_load_tool.common.typing import StrAny
from data_load_tool.common.reflection.utils import evaluate_node_literal
from data_load_tool.common.pipeline import LoadInfo, TPipelineState, get_dlt_repos_dir
...
from data_load_tool.pipeline.trace import PipelineTrace
...
class BaseDeployment(abc.ABC):
        ...
        with get_repo(self.pipeline_script_path) as repo:
            ...
            with set_working_dir(self.working_directory):
                ...
                if not self.pipeline_name:
                    self.pipeline_name = data_load_tool.config.get("pipeline_name")
                    if not self.pipeline_name:
                        ...
                        fmt.warning(
                            ...
                            " is not passed as argument to data_load_tool.pipeline nor configured via config"
    ...
    def _display_missing_secret_info(self) -> None:
        fmt.warning(
            "We could not read and display some secrets. Starting from 1.0 version of data_load_tool,"
    ...
    def _lookup_secret_value(self, trace: LookupTrace) -> Any:
        return data_load_tool.secrets[StringTomlProvider.get_key_name(trace.key, *trace.sections)]
...
def parse_pipeline_info(visitor: PipelineScriptVisitor) -> List[Tuple[str, Optional[str]]]:
    ...
    if n.PIPELINE in visitor.known_calls:
        for call_args in visitor.known_calls[n.PIPELINE]:
            ...
            # Check both full_refresh/dev_mode until full_refresh option is removed from data_load_tool
            ...
            if f_r_node:
                ...
                if f_r_value is None:
                    fmt.warning(
                        "The value of `dev_mode` in call to `data_load_tool.pipeline` cannot be"
...
def generate_pip_freeze(requirements_blacklist: List[str], requirements_file_name: str) -> str:
    ...
    if conflicts:
        ...
        return "# please provide valid dependencies including data_load_tool package"
