...
def _create_parser() -> Tuple[argparse.ArgumentParser, Dict[str, SupportsCliCommand]]:
    parser = argparse.ArgumentParser(
        description=(
            "Creates, adds, inspects and deploys data_load_tool pipelines. Further help is available at"
...
def main() -> int:
    ...
    if Venv.is_virtual_env() and not Venv.is_venv_activated():
        fmt.warning(
            "You are running data_load_tool installed in the global environment, however you have virtual"
            " environment activated. The data_load_tool command will not see dependencies from virtual"
            " environment. You should uninstall the data_load_tool from global environment and install it in"
