...
from data_load_tool.common.utils import get_callable_name, get_full_callable_name, is_inner_callable
from data_load_tool.destinations.impl.destination.factory import destination as _destination
from data_load_tool.destinations.impl.destination.configuration import (
...
def destination(
    ...
    >>> @data_load_tool.destination(batch_size=100, loader_file_format="parquet")
    >>> def my_destination(items, table, api_url: str = data_load_tool.config.value, api_secret = data_load_tool.secrets.value):
    >>>     print(table["name"])
    >>>     print(items)
    >>>
    >>> p = data_load_tool.pipeline("chess_pipeline", destination=my_destination)

    Here all incoming data will be sent to the destination function with the items in the requested format and the data_load_tool table schema.
    ...
    Returns:
        A callable that can be used to create a data_load_tool custom destination instance
    """
    ...
    # we're called as @data_load_tool.destination without parens.
