...
from data_load_tool.common import logger, time, json, pendulum
from data_load_tool.common.destination.utils import resolve_merge_strategy
from data_load_tool.common.metrics import LoadJobMetrics
from data_load_tool.common.schema.typing import C_DLT_LOAD_ID, TTableSchemaColumns
from data_load_tool.common.storages.fsspec_filesystem import glob_files
from data_load_tool.common.typing import DictStrAny
from data_load_tool.common.schema import Schema, TSchemaTables
from data_load_tool.common.schema.utils import get_columns_names_with_prop
from data_load_tool.common.storages import FileStorage, fsspec_from_config
...
from data_load_tool.destinations.sql_client import WithSqlClient, SqlClientBase
...
from data_load_tool.common.destination.dataset import SupportsReadableRelation
from data_load_tool.common.destination.exceptions import DestinationUndefinedEntity

from data_load_tool.destinations.job_impl import (
...
from data_load_tool.destinations import path_utils
from data_load_tool.destinations.fs_client import FSClientBase
from data_load_tool.destinations.utils import verify_schema_merge_disposition
...
class FilesystemLoadJob(RunnableLoadJob):
    ...
    def make_remote_path(self) -> str:
        ...
        destination_file_name = path_utils.create_path(
            ...
            load_package_timestamp=data_load_tool.current.load_package()["state"]["created_at"],
...
class DeltaLoadFilesystemJob(TableFormatLoadFilesystemJob):
    def run(self) -> None:
        ...
        from data_load_tool.common.libs.deltalake import write_delta_table, merge_delta_table
    ...
    def _storage_options(self) -> Dict[str, str]:
        from data_load_tool.common.libs.deltalake import _deltalake_storage_options
    ...
    def _delta_table(self) -> Optional["DeltaTable"]:  # type: ignore[name-defined] # noqa: F821
        from data_load_tool.common.libs.deltalake import DeltaTable
...
class IcebergLoadFilesystemJob(TableFormatLoadFilesystemJob):
    def run(self) -> None:
        from data_load_tool.common.libs.pyiceberg import write_iceberg_table
    ...
    def _iceberg_table(self) -> "pyiceberg.table.Table":  # type: ignore[name-defined] # noqa: F821
        from data_load_tool.common.libs.pyiceberg import get_catalog
...
class FilesystemClient(
    ...
    def sql_client(self) -> SqlClientBase[Any]:
        ...
        from data_load_tool.destinations.impl.filesystem.sql_client import FilesystemSqlClient
    ...
    def get_table_dir(self, table_name: str, remote: bool = False) -> str:
        # data_load_tool tables do not respect layout (for now)
    ...
    def get_table_prefix(self, table_name: str) -> str:
        ...
        if table_name.startswith(self.schema._dlt_tables_prefix):
            # data_load_tool tables get layout where each tables is a folder
        ...
        if table_format in ("delta", "iceberg"):
            ...
            if ReferenceFollowupJobRequest.is_reference_job(file_path):
                if table_format == "delta":
                    import data_load_tool.common.libs.deltalake

                    return DeltaLoadFilesystemJob(file_path)
                elif table_format == "iceberg":
                    import data_load_tool.common.libs.pyiceberg
