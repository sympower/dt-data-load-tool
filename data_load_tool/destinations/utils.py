...
from data_load_tool.common.schema.exceptions import SchemaCorruptedException
from data_load_tool.common.schema.typing import MERGE_STRATEGIES, TColumnType, TTableSchema
...
from data_load_tool.destinations.exceptions import DatabaseTransientException
from data_load_tool.extract import DltResource, resource as make_resource, DltSource
            ...
            if merge_strategy == "delete-insert":
                if not has_column_with_prop(table, "primary_key") and not has_column_with_prop(
                    ...
                    log(
                        ...
                        " merge keys defined."
                        " data_load_tool will fall back to `append` for this table."
                    )
...
def is_compression_disabled() -> bool:
    from data_load_tool import config
