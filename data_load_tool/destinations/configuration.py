...
from data_load_tool.common.typing import Annotated
from data_load_tool.common.configuration.specs.base_configuration import NotResolved
from data_load_tool.common.destination.client import DestinationClientConfiguration
...
class WithLocalFiles(DestinationClientConfiguration):
    ...
    def on_partial(self) -> None:
        if not self.local_dir:
            self.local_dir = os.path.abspath(data_load_tool.current.run_context().local_dir)
            self.resolve()
    ...
    def make_location(self, configured_location: str, default_location_pat: str) -> str:
        ...
        if configured_location == ":pipeline:":
            ...
            #             " database in secrets and changing the current working folder so  data_load_tool"
