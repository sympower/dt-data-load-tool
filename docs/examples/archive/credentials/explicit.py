...
def simple_data(
    api_url: str = data_load_tool.config.value, api_secret: data_load_tool.TSecretValue = data_load_tool.secrets.value
) -> Iterator[str]:
...
data = simple_data(data_load_tool.config["custom.simple_data.api_url"], data_load_tool.secrets["simple_data_secret_7"])
...
pipeline = data_load_tool.pipeline(
    destination=postgres(credentials=data_load_tool.secrets["custom.destination.credentials"])
)
