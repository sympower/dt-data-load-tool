...
Currently, MySQL and SQLite are considered to have full support and are tested as part of the `data_load_tool` CI suite. Other dialects are not tested but should generally work.

## Install data_load_tool with SQLAlchemy

Install data_load_tool with the `sqlalchemy` extra dependency:

```sh
pip install "data_load_tool[sqlalchemy]"
```
...
```sh
data_load_tool init chess sqlalchemy
```
...
pipeline = data_load_tool.pipeline(
    pipeline_name='chess',
    destination=data_load_tool.destinations.sqlalchemy(engine),
    dataset_name='main'
)
```
...
This engine is not disposed of automatically by `data_load_tool`. Example:
...
pipeline = data_load_tool.pipeline('my_pipeline', destination=data_load_tool.destinations.sqlalchemy(engine), dataset_name='main')
...
1. `data_load_tool` must be able to recognize if a database exception relates to non existing entity (like table or schema). We put
...
All `data_load_tool` data types are supported, but how they are stored in the database depends on the SQLAlchemy dialect.
