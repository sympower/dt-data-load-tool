---
title: 🧪 Dremio
description: Dremio `data_load_tool` destination
keywords: [dremio, iceberg, aws, glue catalog]
---

# Dremio

## Install data_load_tool with Dremio
**To install the data_load_tool library with Dremio and s3 dependencies:**
```sh
pip install "data_load_tool[dremio,s3]"
```
...
Let's start by initializing a new data_load_tool project as follows:
   ```sh
   data_load_tool init chess dremio
   ```
...
```
or with `pip install "data_load_tool[dremio,s3]"` which will install `s3fs`, `pyarrow`, and `botocore` packages.

To edit the `data_load_tool` credentials file with your secret info, open `.data_load_tool/secrets.toml`. You will need to provide a `bucket_url` which holds the uploaded parquet files.
...
Therefore, "Metastore" data sources, such as Hive or Glue, require that the dataset schema exists prior to running the data_load_tool pipeline. `dev_mode=True` is unsupported for these data sources.

"Object Storage" data sources do not have this limitation.

## Staging support

Using a staging destination is mandatory when using the Dremio destination. If you do not set staging to `filesystem`, data_load_tool will automatically do this for you.
