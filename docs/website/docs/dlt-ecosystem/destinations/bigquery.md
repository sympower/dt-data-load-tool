...
description: Google BigQuery `data_load_tool` destination
keywords: [bigquery, destination, data warehouse]
---

# Google BigQuery

## Install data_load_tool with BigQuery

**To install the data_load_tool library with BigQuery dependencies:**

```sh
pip install "data_load_tool[bigquery]"
```
...
```sh
data_load_tool init chess bigquery
```
...
This will install data_load_tool with the `bigquery` extra, which contains all the dependencies required by the BigQuery client.
...
Click the `Continue` button and grant the following roles, so that `data_load_tool` can create schemas and load data:
...
**7. Update your `data_load_tool` credentials file with your service account info**

Open your `data_load_tool` credentials file:

```sh
open .data_load_tool/secrets.toml
```
...
`data_load_tool` uses `BigQuery` load jobs that send files from the local filesystem or GCS buckets.
...
You can let BigQuery infer schemas and create destination tables instead of `data_load_tool`. As a consequence, nested fields (i.e., `RECORD`), which `data_load_tool` does not support at
...
```
We recommend yielding [Arrow tables](../verified-sources/arrow-pandas.md) from your resources and using the Parquet file format to load the data. In that case, the schemas generated by `data_load_tool` and BigQuery
...
@data_load_tool.resource(name="cve")
def load_cve():
  with open("cve.json", 'rb') as f:
    # autodetect arrow schema and yield arrow table
    yield paj.read_json(f)

pipeline = data_load_tool.pipeline("load_json_struct", destination="bigquery")
pipeline.run(
...
`data_load_tool` not being able to infer nested fields.
:::
...
:::caution
**BigQuery cannot load JSON columns from Parquet files**. `data_load_tool` will fail such jobs permanently. Instead:
...
BigQuery uses case-sensitive identifiers by default, and this is what `data_load_tool` assumes. If the dataset you use has case-insensitive identifiers (you have such an option
when you create it), make sure that you use a case-insensitive [naming convention](../../general-usage/naming-convention.md#case-sensitive-and-insensitive-destinations) or you tell `data_load_tool` about it so identifier collisions are properly detected.
```toml
[destination.bigquery]
has_case_sensitive_identifiers=false
```

You have an option to allow `data_load_tool` to set the case sensitivity for newly created datasets. In that case, it will follow the case sensitivity of the current
...
BigQuery supports GCS as a file staging destination. `data_load_tool` will upload files in the parquet format to GCS and ask BigQuery to copy their data directly into the database.
...
In the case of implicit credentials (i.e., available in a cloud function), `data_load_tool` shares the `project_id` and delegates obtaining credentials to the `dbt` adapter.
...
Refer to the [full API specification](../../api_reference/data_load_tool/destinations/impl/bigquery/bigquery_adapter) for more details.
:::
