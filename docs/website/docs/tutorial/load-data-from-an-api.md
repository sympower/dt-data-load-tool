---
title: "Build a data_load_tool pipeline"
description: Build a data pipeline with data_load_tool
keywords: [getting started, quick start, basic examples]
---

This tutorial introduces you to foundational data_load_tool concepts, demonstrating how to build a custom data pipeline that loads data from pure Python data structures to DuckDB. It starts with a simple example and progresses to more advanced topics and usage scenarios.
...
1. Import the `data_load_tool` library.
...
`data_load_tool` just created a database schema called **mydata** (the `dataset_name`) with a table **users** in it.

### Explore data in Python

You can use data_load_tool [datasets](../general-usage/dataset-access/dataset) to easily query the data in pure Python.
...
```sh
data_load_tool pipeline quick_start show
```
...
:::tip
`data_load_tool` works in Jupyter Notebook and Google Colab! See our [Quickstart Colab Demo.](https://colab.research.google.com/drive/1NfSB1DpwbbHX9_t5vlalBTf13utwpMGx?usp=sharing)

Looking for the source code of all the snippets? You can find and run them [from this repository](https://github.com/dlt-hub/data_load_tool/blob/devel/docs/website/docs/getting-started-snippets.py).
:::

Now that you have a basic understanding of how to get started with data_load_tool, you might be eager to dive deeper. For that, we need to switch to a more advanced data source - the GitHub API. We will load issues from our [dlt-hub/data_load_tool](https://github.com/dlt-hub/data_load_tool) repository.

:::note
This tutorial uses the GitHub REST API for demonstration purposes only. If you need to read data from a REST API, consider using data_load_tool's REST API source. Check out the [REST API source tutorial](./rest-api) for a quick start or the [REST API source reference](../dlt-ecosystem/verified-sources/rest_api) for more details.
:::

## Create a pipeline

First, we need to create a [pipeline](../general-usage/pipeline). Pipelines are the main building blocks of `data_load_tool` and are used to load data from sources to destinations. Open your favorite text editor and create a file called `github_issues.py`. Add the following code to it:
...
2. Then, it creates a data_load_tool pipeline with the name `github_issues` and specifies that the data should be loaded to the `duckdb` destination and the `github_data` dataset. Nothing gets loaded yet.
...
```sh
data_load_tool pipeline github_issues show
```
...
One option is to tell `data_load_tool` to replace the data in existing tables in the destination by using the `replace` write disposition. Change the `github_issues.py` script to the following:
...
`data_load_tool` will migrate your tables!
...
You can pass a generator to the `run` method directly or use the `@data_load_tool.resource` decorator to turn the generator into a [data_load_tool resource](../general-usage/resource). The decorator allows you to specify the loading behavior and relevant resource parameters.
...
We use the `@data_load_tool.resource` decorator to declare the table name into which data will be loaded and specify the `append` write disposition.

We request issues for the dlt-hub/data_load_tool repository ordered by the **created_at** field (descending) and yield them page by page in the `get_issues` generator function.

We also use `data_load_tool.sources.incremental` to track the `created_at` field present in each issue to filter in the newly created ones.
...
:::tip
Between pipeline runs, `data_load_tool` keeps the state in the same database it loaded data into.
Peek into that state, the tables loaded, and get other information with:

```sh
data_load_tool pipeline -v github_issues_incremental info
```
:::
...
- [`data_load_tool` command line interface.](../reference/command-line-interface)
...
Above, we add the `primary_key` argument to the `data_load_tool.resource()` that tells `data_load_tool` how to identify the issues in the database to find duplicates whose content it will merge.
...
In the previous examples, we used the `requests` library to make HTTP requests to the GitHub API and handled pagination manually. `data_load_tool` has a built-in [REST client](../general-usage/http/rest-client.md) that simplifies API requests. We'll use the `paginate()` helper from it for the next example. The `paginate` function takes a URL and optional parameters (quite similar to `requests`) and returns a generator that yields pages of data.
...
def get_issues(
    ...
    for page in paginate(
        "https://api.github.com/repos/dlt-hub/data_load_tool/issues",
        params={
...
If you want to take full advantage of the `data_load_tool` library, then we strongly suggest that you build your sources out of existing building blocks:
To make the most of `data_load_tool`, consider the following:

## Use source decorator

In the previous step, we loaded issues from the GitHub API. Now we'll load comments from the API as well. Here's a sample [data_load_tool resource](../general-usage/resource) that does that:
...
def get_comments(
    ...
    for page in paginate(
        "https://api.github.com/repos/dlt-hub/data_load_tool/comments",
        params={"per_page": 100}
    ):
        yield page
```

We can load this resource separately from the issues resource; however, loading both issues and comments in one go is more efficient. To do that, we'll use the `@data_load_tool.source` decorator on a function that returns a list of resources:
...
You've noticed that there's a lot of code duplication in the `get_issues` and `get_comments` functions. We can reduce that by extracting the common fetching code into a separate function and using it in both resources. Even better, we can use `data_load_tool.resource` as a function and pass it the `fetch_github_data()` generator function directly. Here's the refactored code:

```py
import data_load_tool
from data_load_tool.sources.helpers.rest_client import paginate

BASE_GITHUB_URL = "https://api.github.com/repos/dlt-hub/data_load_tool"
...
For the next step, we'd want to get the [number of repository clones](https://docs.github.com/en/rest/metrics/traffic?apiVersion=2022-11-28#get-repository-clones) for our data_load_tool repo from the GitHub API. However, the `traffic/clones` endpoint that returns the data requires [authentication](https://docs.github.com/en/rest/overview/authenticating-to-the-rest-api?apiVersion=2022-11-28).
...
It's a good start. But we'd want to follow the best practices and not hardcode the token in the script. One option is to set the token as an environment variable, load it with `os.getenv()`, and pass it around as a parameter. data_load_tool offers a more convenient way to handle secrets and credentials: it lets you inject the arguments using a special `data_load_tool.secrets.value` argument value.
...
When you add `data_load_tool.secrets.value` as a default value for an argument, `data_load_tool` will try to load and inject this value from different configuration sources in the following order:

1. Special environment variables.
2. `secrets.toml` file.

The `secrets.toml` file is located in the `~/.data_load_tool` folder (for global configuration) or in the `.data_load_tool` folder in the project folder (for project-specific configuration).

Let's add the token to the `~/.data_load_tool/secrets.toml` file:
...
The next step is to make our data_load_tool GitHub source reusable so it can load data from any GitHub repo. We'll do that by changing both the `github_source()` and `fetch_github_data()` functions to accept the repo name as a parameter:
...
def github_source_with_token_and_repo(
    repo_name: str = data_load_tool.config.value,
    access_token: str = data_load_tool.secrets.value,
):
    for endpoint in ["issues", "comments", "traffic/clones"]:
        params = {"per_page": 100}
...
Next, create a `.data_load_tool/config.toml` file in the project folder and add the `repo_name` parameter to it:

```toml
[github_with_source_secrets]
repo_name = "dlt-hub/data_load_tool"
```
...
2. Dive deeper into how data_load_tool works by reading the [Using data_load_tool](../general-usage) section. Some highlights:
    - [Set up "last value" incremental loading](../general-usage/incremental-loading#incremental-loading-with-a-cursor-field).
...
4. Explore the [Examples](../examples) section to see how data_load_tool can be used in real-world scenarios.
