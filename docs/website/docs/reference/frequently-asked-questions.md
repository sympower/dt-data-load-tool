...
1. During resource definition.
   ```py
   @data_load_tool.resource(columns={"column_name": {"data_type": "json"}})
   def my_resource():
...
This method ensures you obtain the full schema details, including all columns, as seen by `data_load_tool`, without needing a predefined contract on these resources.
...
You can use `data_load_tool.sources.incremental` to create a custom cursor for tracking pagination in data streams that lack a specific cursor field. An example can be found in the [Incremental loading with a cursor](../general-usage/incremental-loading.md#incremental-loading-with-a-cursor-field).
...
```py
state = data_load_tool.current.resource_state()
