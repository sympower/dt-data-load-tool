---
title: Resource
description: Explanation of what a data_load_tool resource is
keywords: [resource, api endpoint, data_load_tool.resource]
---
...
A [resource](glossary.md#resource) is an ([optionally async](../reference/performance.md#parallelism-within-a-pipeline)) function that yields data. To create a resource, we add the `@data_load_tool.resource` decorator to that function.
...
```py
@data_load_tool.resource(name='table_name', write_disposition='replace')
def generate_rows():
	for i in range(10):
...
`data_load_tool` will infer the [schema](schema.md) for tables associated with resources from the resource's data.
...
:::tip
You can mark some resource arguments as [configuration and credentials](credentials) values so `data_load_tool` can pass them automatically to your functions.
:::

### Put a contract on tables, columns, and data
Use the `schema_contract` argument to tell data_load_tool how to [deal with new tables, data types, and bad data types](schema-contracts.md). For example, if you set it to **freeze**, `data_load_tool` will not allow for any new tables, columns, or data types to be introduced to the schema - it will raise an exception. Learn more about available contract modes [here](schema-contracts.md#setting-up-the-contract).
...
@data_load_tool.resource(name="user", columns=User)
def get_users():
    ...
```
...
@data_load_tool.resource(name="user", columns=UserWithNesting)
def get_users():
    ...
```

`"skip_nested_types"` omits any `dict`/`list`/`BaseModel` type fields from the schema, so data_load_tool will fall back on the default
...
argument on `data_load_tool.resource`. You could pass the table name as a function with the data item as an
...
@data_load_tool.resource(table_name=lambda event: event['type'])
...
def repo_events() -> Iterator[TDataItems]:
    ...
    yield data_load_tool.mark.with_table_name(item, item["type"])
```
...
so `data_load_tool` can pass them automatically to your functions.

### Process resources with `data_load_tool.transformer`
...
# Get Bulbasaur and Ivysaur (you need data_load_tool 0.4.6 for the pipe operator working with lists).
print(list([1,2] | pokemon()))
```
:::

### Declare a standalone resource
A standalone resource is defined on a function that is top-level in a module (not an inner function) that accepts config and secrets values. Additionally, if the `standalone` flag is specified, the decorated function signature and docstring will be preserved. `data_load_tool.resource` will just wrap the decorated function, and the user must call the wrapper to get the actual resource. Below we declare a `filesystem` resource that must be called before use.
```py
@data_load_tool.resource(standalone=True)
def fs_resource(bucket_url=data_load_tool.config.value):
...
```py
@data_load_tool.resource(standalone=True, name=lambda args: args["stream_name"])
...
You can limit how deep `data_load_tool` goes when generating nested tables and flattening dicts into columns. By default, the library will descend
and generate nested tables for all nested lists, without limit.

:::note
`max_table_nesting` is optional so you can skip it, in this case, data_load_tool will
...
data_load_tool.pipeline(destination="duckdb").run(my_resource().add_limit(10))
```
...
```py
data_load_tool.pipeline(destination="duckdb").run(my_resource().add_limit(max_time=10))
```

You can also apply a combination of both limits. In this case the extraction will stop as soon as either limit is reached.

```py
data_load_tool.pipeline(destination="duckdb").run(my_resource().add_limit(max_items=10, max_time=10))
```
...
def sql_table(credentials, schema, table):
    ...
    for idx, batch in enumerate(table_rows(engine, table_obj)):
      if idx == 0:
        # Emit the first row with hints, table_to_columns and _get_primary_key are helpers that extract data_load_tool schema from
        # SqlAlchemy model
        yield data_load_tool.mark.with_hints(
            batch,
            data_load_tool.mark.make_hints(columns=table_to_columns(table_obj), primary_key=_get_primary_key(table_obj)),
        )
      else:
...
In the example above, we use `data_load_tool.mark.with_hints` and `data_load_tool.mark.make_hints` to emit columns and primary key with the first extracted item. The table schema will be adjusted after the `batch` is processed in the extract pipeline but before any schema contracts are applied, and data is persisted in the load package.
...
@data_load_tool.transformer(columns=columns)
def orders(items: Iterator[FileItemDict]):
  for item in items:
    # copy the file locally
      ...
      # tell data_load_tool to import the dest_file as `csv`
      yield data_load_tool.mark.with_file_import(dest_file, "csv")
...
```
In the example above, we glob all zipped csv files present on **my_bucket/csv/today** (using the `filesystem` verified source) and send file descriptors to the `orders` transformer. The transformer downloads and imports the files into the extract package. At the end, `data_load_tool` sends them to Snowflake (the table will be created because we use `column` hints to define the schema).

If imported `csv` files are not in `data_load_tool` [default format](../dlt-ecosystem/file-formats/csv.md#default-settings), you may need to pass additional configuration.
```toml
...
You can sniff the schema from the data, i.e., using DuckDB to infer the table schema from a CSV file. `data_load_tool.mark.with_file_import` accepts additional arguments that you can use to pass hints at runtime.

:::note
* If you do not define any columns, the table will not be created in the destination. `data_load_tool` will still attempt to load data into it, so if you create a fitting table upfront, the load process will succeed.
...
You can pass individual resources or a list of resources to the `data_load_tool.pipeline` object. The resources loaded outside the source context will be added to the [default schema](schema.md) of the pipeline.
...
```py
@data_load_tool.resource(file_format="parquet")
def generate_rows(nr):
    for i in range(nr):
        yield {'id': i, 'example_string': 'abc'}
```
