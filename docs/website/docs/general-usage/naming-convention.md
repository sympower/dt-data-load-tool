...
`data_load_tool` groups tables from a single [source](source.md) in a [schema](schema.md). Each schema defines a **naming convention** that tells `data_load_tool` how to translate identifiers to the
...
You can pick which naming convention to use. `data_load_tool` provides a few to [choose from](#available-naming-conventions). You can [easily add your own](#write-your-own-naming-convention) as well.

:::tip
The standard behavior of `data_load_tool` is to **use the same naming convention for all destinations** so users always see the same table and column names in their databases.
:::
...
2. Add the following line to your global `config.toml` (the one in your home dir, i.e., `~/.data_load_tool/config.toml`):
```toml
[schema]
naming="sql_ci_v1"
```
...
`data_load_tool` keeps source (not normalized) identifiers during data [extraction](../reference/explainers/how-dlt-works.md#extract) and translates them during [normalization](../reference/explainers/how-dlt-works.md#normalize). For you, it means:
...
2. If you define a `primary_key` or `cursor` that participates in [cursor field incremental loading](incremental-loading.md#incremental-loading-with-a-cursor-field), use the source identifiers (`data_load_tool` uses them to inspect source data, `Incremental` class is just a filtering function).
3. When defining any other hints, i.e., `columns` or `merge_key`, you can pick source or destination identifiers. `data_load_tool` normalizes all hints together with your data.
4. The `Schema` object (i.e., obtained from the pipeline or from `data_load_tool` source via `discover_schema`) **always contains destination (normalized) identifiers**.

### Understand the identifier normalization
Identifiers are translated from source to destination form in the **normalize** step. Here's how `data_load_tool` picks the naming convention:
...
* `data_load_tool` applies the final naming convention in the `normalize` step. Jobs (files) in the load package now have destination identifiers. The pipeline schema is duplicated, locked, and saved in the load package and will be used by the destination.

:::caution
If you change the naming convention and `data_load_tool` detects a change in the destination identifiers for tables/collections/files that already exist and store data, the normalize process will fail. This prevents an unwanted schema migration. New columns and tables will be created for identifiers that changed.
:::

### Case-sensitive and insensitive destinations
Naming conventions declare if the destination identifiers they produce are case-sensitive or insensitive. This helps `data_load_tool` to [generate case-sensitive / insensitive identifiers for the destinations that support both](destination.md#control-how-dlt-creates-table-column-and-other-identifiers). For example, if you pick a case-insensitive naming like **snake_case** or **sql_ci_v1**, with Snowflake, `data_load_tool` will generate all uppercase identifiers that Snowflake sees as case-insensitive. If you pick a case-sensitive naming like **sql_cs_v1**, `data_load_tool` will generate quoted case-sensitive identifiers that preserve identifier capitalization.

Note that many destinations are exclusively case-insensitive, of which some preserve the casing of identifiers (i.e., **duckdb**) and some will case-fold identifiers when creating tables (i.e., **Redshift**, **Athena** do lowercase on the names). `data_load_tool` is able to detect resulting identifier [collisions](#avoid-identifier-collisions) and stop the load process before data is mangled.

### Identifier shortening
Identifier shortening happens during normalization. `data_load_tool` takes the maximum length of the identifier from the destination capabilities and will trim the identifiers that are too long. The default shortening behavior generates short deterministic hashes of the source identifiers and places them in the middle of the destination identifier. This (with a high probability) avoids shortened identifier collisions.

### Compound (flattened) identifiers
`data_load_tool` combines several identifiers in order to name nested tables and flattened columns. For example:
```json
{
  "column":
    {
...
`data_load_tool` version above 1.4.0 normalization is fully idempotent and normalized
`column__value` will be still `column__value`.
:::

:::caution
Previously double underscores were contracted into single underscore. That
prevented using data loaded by `data_load_tool` as a data source without identifier modifications. `data_load_tool` maintains backward compatibility for version >1.4.0 as follows:
...
```yaml
normalizers:
  names: data_load_tool.common.normalizers.names.snake_case
  use_break_path_on_normalize: true
  json:
    module: data_load_tool.common.normalizers.json.relational
```
...
```
`data_load_tool` will import `tests.common.cases.normalizers.sql_upper` and use the `NamingConvention` class found in it as the naming convention.
...
pipeline = data_load_tool.pipeline(dataset_name="MyCamelCaseName")
```
...
`data_load_tool` detects various types of identifier collisions and ignores the others.
1. data_load_tool detects collisions if a case-sensitive naming convention is used on a case-insensitive destination.
2. data_load_tool detects collisions if a change of naming convention changes the identifiers of tables already created in the destination.
3. data_load_tool detects collisions when the naming convention is applied to column names of arrow tables.

`data_load_tool` will not detect a collision when normalizing source data. If you have a dictionary, keys will be merged if they collide after being normalized.
...
Custom naming conventions are classes that derive from `NamingConvention`, which you can import from `data_load_tool.common.normalizers.naming`. We recommend the following module layout:
...
:::note
Note that the fully qualified name of your custom naming convention will be stored in the schema, and data_load_tool will attempt to import it when the schema is loaded from storage.
