...
## importing data_load_tool
Basic `data_load_tool` functionalities are imported with `import data_load_tool`. Those functionalities are:
1. ability to run the pipeline (which means extract->normalize->load for particular source(s) and destination) with `data_load_tool.run`
2. ability to configure the pipeline ie. provide alternative pipeline name, working directory, folders to import/export schema and various flags: `data_load_tool.pipeline`
3. ability to decorate sources (`data_load_tool.source`) and resources (`data_load_tool.resource` , `data_load_tool.transformer`)
4. ability to access secrets `data_load_tool.secrets` and config values `data_load_tool.config`
...
☮️ the built in sources can be imported from `data_load_tool.sources` namespaces
⛔ the contributed sources can be added as python code modules via `data_load_tool init <source> <destination>`

## default and explicitly configured pipelines
When the `data_load_tool` is imported a default pipeline is automatically created. That pipeline is configured via configuration providers (ie. `config.toml` or env variables - see [secrets_and_config.md](secrets_and_config.md)). If no configuration is present, default values will be used.
...
2. the working directory of the pipeline will be (1) for non-root user with home directory (99% of cases) **~/.data_load_tool/pipelines/pipeline name** (2) for root users (Linux/Mac OS): **/var/data_load_tool/pipelines/pipeline name** (3) for users without home directory: **OS temp dir/data_load_tool/pipelines/pipeline name**`
...
Pipeline can be explicitly created and configured via `data_load_tool.pipeline()` that returns `Pipeline` object. All parameters are optional. If no parameter is provided then default pipeline is returned. Here's a list of options. All the options are configurable.
...
> **Achtung** as per `secrets_and_config.md` the arguments passed to `data_load_tool.pipeline` are configurable and if skipped will be injected by the config providers. **the values provided explicitly in the code have a full precedence over all config providers**
...
> It is possible to have several pipelines in a single script if many pipelines are configured via `data_load_tool.pipeline()`.

## the default schema and the default dataset name
`data_load_tool` has following rules when auto-generating schemas and naming the dataset to which the data will be loaded.
...
1. From the first extracted `@data_load_tool.source` ie. if you `data_load_tool.run(data=spotify(), ...)` and `spotify` source has schema with name `spotify` attached, it will be used as default schema.
2. it will be created from scratch if you extract a `@data_load_tool.resource` or an iterator ie. list (example: `data_load_tool.run(data=["a", "b", "c"], ...)`) and its name is the pipeline name or generator function name if generator is extracted. (I'm trying to be smart with automatic naming)
...
p = data_load_tool.pipeline(dataset_name="spotify_data_1")
...
In case **there's only (default) schema** in the pipeline, the data will be loaded into dataset with dataset name. Example: `data_load_tool.run(spotify("me"), dataset_name="spotify_data_1")` will load data into dataset `spotify_data_1`).
...
The `restore_from_destination` argument to `data_load_tool.pipeline` let's the user restore the state and the pipeline schemas from the destination. `data_load_tool` will check if the state is stored in the destination, download it and then download all the required schemas. after that data extraction may be restarted.
...
The other way to trigger full refresh is to drop destination dataset. `data_load_tool` detects that and resets the pipeline local working folder.

## Data sources understood by `data_load_tool`
The `run` and `extract` method understand the following
1. `data_load_tool.source`, `data_load_tool.resource` and lists of them
...
## running pipelines and `data_load_tool.run` + `@source().run` functions
`data_load_tool.run` + `@source().run` are shortcuts to `Pipeline::run` method on default or last configured (with `data_load_tool.pipeline`) `Pipeline` object. Please refer to [create_pipeline.md](create_pipeline.md) for examples.

The function takes the following parameters
1. data - required - the data to be loaded into destination: a `@data_load_tool.source` or a list of those, a `@data_load_tool.resource` or a list of those, an iterator/generator function or a list of those or iterable (ie. a list) holding something else that iterators.
2. destination
3. dataset name
...
### Dev mode mode
If `dev_mode` flag is passed to `data_load_tool.pipeline` then
...
> should we add it? I have a runner in `data_load_tool` that would be easy to modify


## pipeline runtime setup

1. logging - creates logger with the name `data_load_tool` which can be disabled the python way if someone does not like it. (contrary to `dbt` logger which is uncontrollable mess)
2. signals - signals required to gracefully stop pipeline with CTRL-C, in docker, kubernetes, cron are handled. signals are not handled if `data_load_tool` runs as part of `streamlit` app or a notebook.
